...
define file_open_sysconn_bin;
define file_open_sysconn_text;
...
*
*   Create Cognivision file connection handle CONN so that it does I/O to/from
*   system stream STREAM_ID.  RW_MODE specifies the required read/write access
*   to the stream thru CONN.
*
...
********************************************************************************
*
*   Subroutine FILE_OPEN_SYSCONN_BIN (SYSCONN, RW_MODE, CONN, STAT)
*
*   Create file connection handle CONN so that it does I/O to/from the system
*   stream identified by the system I/O connection handle SYSCONN.  RW_MODE
*   specifies the required read/write access to the stream thru CONN.
*
*   The new connection will support binary read/write thru routines
*   FILE_WRITE_BIN and FILE_READ_BIN.
}
...
begin
  ...
  conn.sys := sysconn;                 {save system I/O handle}
...
********************************************************************************
*
*   Subroutine FILE_OPEN_SYSCONN_TEXT (SYSCONN, RW_MODE, CONN, STAT)
*
*   Create file connection handle CONN so that it does I/O to/from the system
*   stream identified by the system I/O connection handle SYSCONN.  RW_MODE
*   specifies the required read/write access to the stream thru CONN.
...
begin
  ...
  file_open_sysconn_bin (sysconn, rw_mode, data_p^.conn, stat); {open as binary}
  if sys_error(stat) then begin        {error opening file ?}
    sys_mem_dealloc (data_p);          {deallocate private data block}
    return;                            {return with error}
    end;
